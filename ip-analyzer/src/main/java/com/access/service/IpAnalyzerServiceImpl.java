...
public class IpAnalyzerServiceImpl implements IpAnalyzerService {
    ...
    public AttackAttemptDto processAuthFailure(AuthFailureDto dto) {
        ...
        if (optionalFailureList.isPresent()) {
            ...
            return attemptsMapProcessing(failureList, dto);
        } else {
    ...
    private AttackAttemptDto attemptsMapProcessing(FailureList failureList, AuthFailureDto dto) {
        ...
        if (recentAttempts.size() < threshold - 1) {
            ...
            if (cacheDb.existsById(dto.subnet())) {
                ...
                return new AttackAttemptDto(dto.subnet(), recentAttempts.lastKey(), services);
            } else {
